function calc() {
  ...
  if(m1+m2+m3+m4+m5+m6+c1+c2+c3+c4+s1+s2+s3+s4+v1+v2+v3+v4+o1+o2+o3+o4>15){
    ...
    if(m1>3||m2>3||m3>3||m4>3||m5>3||m6>3||c1>3||c2>3||c3>3||c4>3||s1>3||s2>3||s3>3||s4>3||v1>3||v2>3||v3>3||v4>3||o1>3||o2>3||o3>3||o4>3||m1<0||m2<0||m3<0||m4<0||m5<0||m6<0||c1<0||c2<0||c3<0||c4<0||s1<0||s2<0||s3<0||s4<0||v1<0||v2<0||v3<0||o1<0||v4<0||o2<0||o3<0||o4<0){ $("#result").text("Мы не можем положить больше трех порций одного ингридиента (Или вы ввели отрицательное число).");}
  else{ 
  if ($("#size").val() == "18cm") {
    size += 200;
  }
