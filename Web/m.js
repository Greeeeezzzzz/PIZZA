function calc() {
  ...
  if(m1+m2+m3+m4+m5+m6+c1+c2+c3+c4+s1+s2+s3+s4+v1+v2+v3+v4+o1+o2+o3+o4>15){
    ...
    if( 0>m1>3||m2>3||m3>3||m4>3||m5>3||m6>3||c1>3||c2>3||c3>3||c4>3||s1>3||s2>3||s3>3||s4>3||v1>3||v2>3||v3>3||v4>3||o1>3||o2>3||o3>3||o4>3){ $("#result").text("Мы не можем положить больше трех порций одного ингридиента.");}
  else{ 
  if ($("#size").val() == "18cm") {
    size += 200;
  }
